...
class AsyncWebSocketClient:
    ...
    def is_connected(self) -> bool:
        return (
            ...
            and self._is_websocket_closed() is False
    ...
    def _is_websocket_closed(self) -> bool:
        """Helper to check if websocket is closed, compatible across versions."""
        if self.websocket is None:
            return True
        # Try common attributes for checking closed state
        for attr in ["closed", "close_code", "connection_lost"]:
            if hasattr(self.websocket, attr):
                val = getattr(self.websocket, attr)
                if isinstance(val, bool):
                    return val
        # Fallback to checking if it's connected
        return False
    ...
    async def connect(self, urls: List[str], ssid: str) -> bool:
        for url in urls:
            try:
                ...
                # Try without forcing TLS 1.3 for compatibility
                ...
                # Try different headers parameters based on websockets version
                import websockets
                try:
                    # Try with additional_headers first
                    connect_kwargs["additional_headers"] = DEFAULT_HEADERS
                    self.websocket = await asyncio.wait_for(
                        websockets.connect(**connect_kwargs),
                        timeout=CONNECTION_SETTINGS.get("handshake_timeout", 10.0)
                    )
                except TypeError as e:
                    if "extra_headers" in str(e) or "additional_headers" in str(e) or "headers" in str(e):
                        # Try with headers parameter instead
