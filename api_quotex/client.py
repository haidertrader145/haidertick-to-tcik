...
class AsyncQuotexClient:
    ...
    def add_event_handler(self, event_name: str, callback: Callable):
        """Add an event handler to listen for events from the websocket."""
        self._websocket.add_event_handler(event_name, callback)
        if self._keep_alive_manager:
            self._keep_alive_manager.add_event_handler(event_name, callback)

    def remove_event_handler(self, event_name: str, callback: Callable):
        """Remove an event handler from the websocket."""
        self._websocket.remove_event_handler(event_name, callback)
        if self._keep_alive_manager:
            self._keep_alive_manager.remove_event_handler(event_name, callback)
